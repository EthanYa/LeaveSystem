...
<body>
		...
		//alert("12333");
			...
			$('#btnUpdate')
					.on(
							...
							function() {
								//alert("12333");
								var adyearName = $('#updateblock').val();
        if(adyearName=="")
        {
         alert("請輸入數字");
                     return false;
 ...
 $(document).ready(function(){
  $('#updateblock').blur(function(){
   ...
   var adyearName = $('#updateblock').val();
   if(adyearName.trim()==""||adyearName.trim()==null){
  ...
  $('#updateblock').keyup(function(){
   ...
   var adyearName = $('#updateblock').val();
   if(adyearName==null){//if  null  lock submit
    $('#btnUpdate').attr('disabled',true);
   }
   else{         //can submit
   ...
   $.ajax({
    ...
    url:'${pageContext.request.contextPath}/ManagerServletAdyear?act=checkad',
    success:function(result){
     //alert("123");
     if(result=="true"){
      $('#errortip').text('無重複，可以使用');
      $('#btnUpdate').attr('disabled',false);
     }else {
				...
				<div class="modal-body">
					<form
						...
						<table class="table table-bordered">
							...
							<tr>
        <th>西元年</th>
        <td><input type="text" id="updateblock"  name="adyear" value="${param.adyear}" />
