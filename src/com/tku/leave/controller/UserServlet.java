		...
		if ("listLeave".equals(act)) {// 顯示假別
			...
			for (int i = 1; i < counter + 1; i++) {// 相同假別請假時數判斷
				...
				if (remain >= leavetimei) {// 時數足夠
					...
					out.print("<script laguage='JavaScript'> alert('超過可請時數，請重新填寫。');location.href='http://localhost:8080/LeaveSystem/jsps/user/LeaveDescription.jsp';</script>");
					out.flush();
					out.close();
				}
				leavetimei = 0;
